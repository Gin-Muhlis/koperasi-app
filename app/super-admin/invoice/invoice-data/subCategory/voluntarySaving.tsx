...
const VoluntarySavingPopup = ({ memberVoluntarySaving, setSubCategory }: { memberVoluntarySaving: SubCategoryInvoice[], setSubCategory: React.Dispatch<React.SetStateAction<string>> }) => {
    const dispatch = useDispatch<appDispatch>();
    const selector = useAppSelector((state) => state.invoiceReducer);
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [itemsPerPage, setItemsPerPage] = useState<number>(10);
    const [members, setMembers] = useState<SubCategoryInvoice[]>(memberVoluntarySaving);
    const [listSimpananSukarela, setListSimpananSukarela] = useState<Member[]>([])
    ...
    useEffect(() => {
        if (selector) {
            const data = JSON.parse(selector.listSimpananSukarela)

            setListSimpananSukarela(data)
        }
    }, [selector, selector.listSimpananSukarela])

    useEffect(() => {
        if (memberVoluntarySaving) {
            const filteredMember = memberVoluntarySaving.filter((member) => member.payment != 0)
    ...
    const filterMembersByPosition = (value: string) => {
        const newMembers = memberVoluntarySaving.filter((member) => member.position == value);
    ...
    const handleAddMember = (id: number) => {
        ...
        if (existingItemIndex >= 0) {
            const data = listSimpananSukarela[existingItemIndex];
            const updatedItems: Member[] = [...listSimpananSukarela];
    ...
    const handleDeleteMember = (id: number) => {
        let newMembers: Member[] = listSimpananSukarela.filter((item: any) => item.id != id);
        setStateData(newMembers)
    };

    //handle tampilan button tabel   
    const handleButtonAdd = (id: number) => {
        const isInputed = listSimpananSukarela.find(
            (item: any) => item.id == id
        );
    ...
    const handleDisableInput = (id: number) => {
        const isAdded = listSimpananSukarela.find(data => data.id == id && data.status == "added");
    ...
    const handleAddAllmember = () => {
        const updatedList: Member[] = [...listSimpananSukarela];

        listSimpananSukarela.map((item) => {
            if (item.status == 'not_added') {
                const index = listSimpananSukarela.findIndex((data) => data.id == item.id)
    ...
    const handleValueAmount = (id: number) => {
        const isInputed = listSimpananSukarela.find((data) => data.id == id)
    ...
    const handleCountData = (type: string) => {
        if (type == 'cancel') {
            const dataAdded = listSimpananSukarela.filter(data => data.status === 'added')

            return dataAdded.length
        } else {
            const dataNotAdded = []
            listSimpananSukarela.map((member) => {
            ...
            <div className={`p-5 fixed inset-0 z-50 w-full min-h-screen bg-black/80 flex items-start justify-center overflow-y-scroll`}>
                <div className="bg-white rounded p-5 w-full">
                    <div className="w-full flex flex-col gap-5 mb-5">
                        <div className="w-1/2">
                            <h1 className="text-black text-xl font-bold mb-3">Simpanan Sukarela</h1>
...
export default VoluntarySavingPopup