...
const Content = ({detailInvoice}: {detailInvoice: InvoiceState}) => {

    const { toPDF, targetRef } = usePDF({ filename: 'Pembayaran.pdf' });
  ...
  return (
    <div className="bg-white rounded p-4 w-full">
        ...
        <div className="w-full flex items-center justify-start gap-2 mb-5">
          ...
          <DownloadPdfInvoiceButton detailInvoice={detailInvoice.details} timeInvoice={convertDateFormat(detailInvoice.due_date)} />
          <DownloadExcelInvoiceButton detailInvoice={detailInvoice.details} timeInvoice={convertDateFormat(detailInvoice.due_date)} />
        </div>
