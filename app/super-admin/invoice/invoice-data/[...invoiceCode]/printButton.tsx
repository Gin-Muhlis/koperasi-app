...
const PrintButton = ({ memberName, memberId, invoiceCode }: { memberName: string, memberId: number, invoiceCode: string }) => {
    ...
    return (
        <>
            <button className='bg-blue-400 text-white p-1 outline-none rounded text-xs cursor-pointer' onClick={handleDownloadInvoice}>{isLoading ? <Loader /> :  <Icon icon="lucide:file" width={16} height={16}></Icon>}</button>
